...
class Events(models.Model):
    ...
    event_type = models.CharField(
        max_length=15, choices=Event_Types, default="individual"
    )
        ...
        try:
            if timezone.is_naive(self.deadline):
                self.deadline = timezone.make_aware(
                    self.deadline, timezone.get_current_timezone()
                )
            return (
                self.current_registration < self.event_capacity
...
class Registration(models.Model):
    ...
    event = models.ForeignKey(
        Events, on_delete=models.CASCADE, related_name="registrations", db_index=True
    )
    ...
    def clean(self):
        ...
        if not self.event.can_register():
            raise ValidationError(
                "Cannot register: event is full or deadline has passed."
            )
        if self.event.paid and not self.payment_screenshot:
            raise ValidationError("Payment screenshot is required for paid events.")
        if self.event.current_registration >= self.event.event_capacity:
...
class TeamMember(models.Model):
    registration = models.ForeignKey(
        Registration, on_delete=models.CASCADE, related_name="team_members"
    )
    name = models.CharField(max_length=250)
    ...
    def __str__(self):
        return (
            f"{self.name} ({self.email}) - Team: {self.registration.team_name or 'N/A'}"
        )
